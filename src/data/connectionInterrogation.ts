...
export function generateConnectionInterrogation(params: {
  ...
  if (isOtSource && isNtTarget) {
    // Mode 1: OT ➔ NT (Prophecy / Typology Fulfillment) — plain voice, §1.7
    who = `Authorship & Context: ${sourceAuthorStr} wrote the Old Testament witness (${anchorRef}); ${targetAuthorStr} recorded its fulfillment (${targetRef}). Identified Characters: Israel's prophets and people, who first received the promise; then Jesus of Nazareth, His apostles, and the early church, who saw it come true. Singular or Many: Many. Many people carry the story, but Jesus Christ is its one subject. Christological Subject & Referent: Jesus Christ. He is the promised Messiah the Old Testament points to and the New Testament names. Redemptive Purpose: God carried one promise across the centuries to show that Jesus is the Savior He always planned to send.`;
    what = `${anchorRef} makes a promise: “${cleanAnchor.slice(0, 110)}...” ${targetRef} shows it happening: “${cleanTarget.slice(0, 110)}...”`;
    when = `The promise was spoken in the Old Testament era of ${anchorRef}. It was fulfilled in the New Testament era of ${targetRef}.`;
    how = `The link is prophecy fulfilled. The Old Testament sets the pattern in Hebrew (for example berit, “covenant”, and go'el, “kinsman-redeemer”). The New Testament shows Jesus stepping into that pattern (for example diathēkē, “covenant”, and apolytrosis, “ransom”).`;
    why = `God repeated this promise across the centuries so that no one could call its arrival a coincidence. The fulfillment proves the promise was His all along.`;
    ultimatePoint = `Jesus is the fulfillment of the promise made in ${anchorRef}. What God began there, He finished in Christ.`;
    personalRelevance = `Why you need to know this: God's promises do not expire. If He kept His word across the centuries, He will keep His word to you. What it does for you: Fulfilled prophecy is proof you can hold onto when life feels uncertain. Relationship with Jesus: You can trust Jesus with your future, because the thread of Scripture shows He has never once dropped it.`;
  } else if (isNtSource && isOtTarget) {
    // Mode 2: NT ➔ OT (Apostolic Retrospective) — plain voice, §1.7
    who = `Authorship & Context: ${sourceAuthorStr} wrote ${anchorRef} in the apostolic era, looking back at Scripture written by ${targetAuthorStr}. Identified Characters: Jesus, His apostles, and the early church, reading Israel's prophets and psalms as His story. Singular or Many: Many. Many writers span the centuries, but they all point to one person. Christological Subject & Referent: Jesus Christ. The Old Testament is the ground He stands on; the New Testament is His explanation. Redemptive Purpose: The apostles quoted Israel's Scriptures to prove the gospel was not a new religion but God's plan arriving on schedule.`;
    what = `${anchorRef} stands on the foundation of ${targetRef}. The apostolic writer reaches back to Israel's Scriptures: “${cleanTarget.slice(0, 110)}...”`;
    when = `${anchorRef} was written in the first century AD. It looks back across the centuries to ${targetRef}.`;
    how = `The link is quotation and explanation. The New Testament writer quotes or recalls the Hebrew text and shows that its meaning is Jesus. Luke 24:44 names “the law of Moses, and the prophets, and the psalms” as the book that is about Him.`;
    why = `To prove that the gospel was God's plan from the start. The prophets announced it; Jesus fulfilled it.`;
    ultimatePoint = `The gospel of Jesus is God's oldest plan: He promised it, the prophets wrote it, and Jesus fulfilled it.`;
    personalRelevance = `Why you need to know this: Your faith does not rest on a recent idea but on God's unbroken word. What it does for you: It gives your trust in Jesus deep roots that doubt cannot pull up. Relationship with Jesus: The same Scriptures that carried believers for centuries now carry you.`;
  } else if (isNtSource && isNtTarget) {
    // Mode 3: NT ➔ NT (Apostolic Harmony) — plain voice, §1.7
    who = `Authorship & Context: ${sourceAuthorStr} and ${targetAuthorStr} wrote within one generation of Jesus, guided by the same Spirit. Identified Characters: The risen Christ, His apostles, and the early churches across the Roman world. Singular or Many: Many. Many churches and writers, one Lord. Christological Subject & Referent: Jesus Christ — the head of the church and the subject of every apostolic letter. Redemptive Purpose: The apostles teach one consistent gospel for everyday life.`;
    what = `${anchorRef} and ${targetRef} teach the same truth in different settings. Together they show the apostles spoke with one voice.`;
    when = `Both passages come from the apostolic era of the first century AD, as the church took root across the Roman world.`;
    how = `The link is agreement. Two writers, working apart, describe the same gospel in the same words — grace (charis), faith (pistis), and fellowship (koinōnia).`;
    why = `So the church would stand on one clear teaching, not on the opinion of this teacher or that.`;
    ultimatePoint = `The apostles' message is one message: Jesus saves, and His grace is enough.`;
    personalRelevance = `Why you need to know this: The Bible does not contradict itself about how you are saved. What it does for you: You can read with confidence instead of confusion. Relationship with Jesus: The same Lord who guided the apostles speaks to you in their words.`;
  } else {
    // Mode 4: OT ➔ OT (Covenant Progression) — plain voice, §1.7
    who = `Authorship & Context: ${sourceAuthorStr} wrote ${anchorRef}; ${targetAuthorStr} carried the story forward in ${targetRef}. Identified Characters: Israel's patriarchs, kings, prophets, and people — and the Redeemer they all await. Singular or Many: Many. Many generations pass the promise along, but it points to one coming Savior. Christological Subject & Referent: The promised Christ, coming closer with every step of Israel's history. Redemptive Purpose: Each passage builds on the last, teaching Israel who God is and preparing the way for Jesus.`;
    what = `The thread grows inside the Old Testament: ${anchorRef} plants it, and ${targetRef} develops it.`;
    when = `Both passages belong to the Old Testament story of Israel, from the patriarchs through the kings, the exile, and the return.`;
    how = `The link is continuity. Hebrew covenant words carry the promise forward: berit (“covenant”), chesed (“steadfast love”), and Mashiach (“Anointed King”).`;
    why = `God taught His people step by step, so the promise would be recognized when it finally arrived.`;
    ultimatePoint = `God kept the thread unbroken through every generation, until the Redeemer came.`;
    personalRelevance = `Why you need to know this: God works patiently, across years and generations. What it does for you: His patience with Israel is patience with you — His timing is worth trusting. Relationship with Jesus: Every Old Testament step is one more proof that Jesus finishes what God starts.`;
  }
