...
- in-scope strings: 70
- rewritten: 19 · VERIFY-ONLY: 51 · EQUIVALENT: 0
- applier: `npx tsx scripts/cp03ApplyRewrites.ts docs/_work/mal_rewrites.json --worklist docs/_work/mal.json --apply`
...
§1.12: chain prose excluded from this per-book apply — 9 chain draft(s), 29 chain verify-only, 0 chain equivalent. They belong to the dedicated chain pass.
=== apply Malachi · docs/_work/mal_rewrites.json ===
drafts: 10 · verify-only: 22 · equivalent: 0
...
--- post-flight (git diff) ---
  verse entries named by drafts:  68
  verse entries git reports:      68
  chain drafts:                   0
...
Drafts 19 across the book; measured on the entry strings only, chain prose excluded per §1.12.
