...
- applier: `npx tsx scripts/cp03ApplyRewrites.ts docs/_work/nam_rewrites.json --worklist docs/_work/nam.json --apply`
...
=== apply Nahum · docs/_work/nam_rewrites.json ===
drafts: 4 · verify-only: 8 · equivalent: 0
...
--- post-flight (git diff) ---
  verse entries named by drafts:  39
  verse entries git reports:      39
  chain drafts:                   0
...
Drafts 4 across the book; measured on the entry strings only, chain prose excluded per §1.12.
