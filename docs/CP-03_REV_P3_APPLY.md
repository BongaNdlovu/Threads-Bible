...
- in-scope strings: 80
- rewritten: 38 · VERIFY-ONLY: 42 · EQUIVALENT: 0
- applier: `npx tsx scripts/cp03ApplyRewrites.ts docs/_work/rev_p3_rewrites.json --worklist docs/_work/rev_p3.json --apply`
...
=== apply Revelation · docs/_work/rev_p3_rewrites.json ===
drafts: 38 · verify-only: 42 · equivalent: 0
worklist strings: 80 · mode: DRY RUN
...
--- post-flight (git diff) ---
  verse entries named by drafts:  82
  verse entries git reports:      82
  chain drafts:                   0
...
## Sweep findings — part 3

Drafts 38 across the book; measured on the entry strings only, chain prose excluded per §1.12.
