# CP-03 · 2 Chronicles — apply record
...
- in-scope strings: 16
- rewritten: 8 · VERIFY-ONLY: 8 · EQUIVALENT: 0
- applier: `npx tsx scripts/cp03ApplyRewrites.ts docs/_work/2ch_rewrites.json --worklist docs/_work/2ch.json --apply`

## Proofs (verbatim command output)

```text
$ npx tsx scripts/cp03ApplyRewrites.ts docs/_work/2ch_rewrites.json --worklist docs/_work/2ch.json --apply
  verse entries named by drafts:  8   verse entries git reports:  8   RESULT: APPLIED.
$ npx tsx scripts/cp03StructuralVerify.ts --book 2ch  -> OVERALL: PASS (exit 0)
$ npx tsx scripts/cp02RewriteQuality.ts ...           -> REWRITTEN 8 · PUNCTUATION-ONLY 0
