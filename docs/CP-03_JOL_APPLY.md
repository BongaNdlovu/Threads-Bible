...
- in-scope strings: 75
- rewritten: 6 · VERIFY-ONLY: 69 · EQUIVALENT: 0
- applier: `npx tsx scripts/cp03ApplyRewrites.ts docs/_work/jol_rewrites.json --worklist docs/_work/jol.json --apply`
...
§1.12: chain prose excluded from this per-book apply — 0 chain draft(s), 55 chain verify-only, 0 chain equivalent. They belong to the dedicated chain pass.
=== apply Joel · docs/_work/jol_rewrites.json ===
drafts: 6 · verify-only: 14 · equivalent: 0
worklist strings: 20 · mode: DRY RUN
...
--- post-flight (git diff) ---
  verse entries named by drafts:  14
  verse entries git reports:      14
  chain drafts:                   0
...
Drafts 6 across the book; measured on the entry strings only, chain prose excluded per §1.12.
